...
def load_db():
    if IS_VERCEL and not os.path.exists(DB_FILE):
        if os.path.exists("data.json"):
            try:
                shutil.copy("data.json", DB_FILE)
            except Exception as e:
                print(f"Error copying initial DB file: {e}")
...
# Dynamic file serving routes for Vercel's read-only environment
@app.get("/static/audio/{filename}")
async def get_audio_file(filename: str):
    tmp_path = os.path.join(AUDIO_DIR, filename)
    local_path = os.path.join("static/audio", filename)
...
def process_interaction(text: str) -> dict:
    if not text:
        ...
        audio_file = text_to_speech(text, output_dir=AUDIO_DIR)
    ...
    # Text to speech
    audio_file = text_to_speech(speech_text, output_dir=AUDIO_DIR)
